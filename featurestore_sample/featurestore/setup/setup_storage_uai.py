...
def grant_user_aad_storage_data_contributor_role(
    credential,
    user_aad_objectId,
    storage_subscription_id,
    storage_resource_group_name,
    ...
    # Get the "Blob Data Contributor" role definition
    role_definitions = authorization_client.role_definitions.list(
        storage_account_id, filter="roleName eq 'Storage Blob Data Contributor'"
    )
    role_definition = next(iter(role_definitions), None)
